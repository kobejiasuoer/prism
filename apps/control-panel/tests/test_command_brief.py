...
class BuildTodayViewIntegrationTest(unittest.TestCase):
    ...
    def test_today_split_routes_include_command_brief(self) -> None:
        ...
        for key in ("mode", "permits", "position_cap", "first_action", "action_lanes", "trust"):
            self.assertIn(key, brief)
        self.assertTrue(brief.get("details_deferred"))
        self.assertEqual((brief.get("links_lazy") or {}).get("details"), "/api/today/command-brief-detail")
        self.assertIn(brief["mode"]["value"], {"defense", "observe", "probe", "offense"})

        detail_response = self.client.get("/api/today/command-brief-detail")
        self.assertEqual(detail_response.status_code, 200)
        detail = detail_response.json().get("command_brief_detail") or {}
        for key in ("judgement_chain", "midday_verify", "forbid_today", "reclassify_when"):
