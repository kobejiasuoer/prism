...
function formalGapSummary(readiness?: ReadinessPayload | null) {
  ...
  if (formalStatus && provider && !provider.token_configured) {
    return {
      ...
      title: "正式源还没有真正接入",
      detail:
        "后端没有读到 PRISM_TUSHARE_TOKEN / TUSHARE_TOKEN；先在本机 .env 配置 token，再刷新正式口径数据。",
  ...
  if (formalStatus && !formalStatus.ready) {
    ...
    return {
      ...
      detail:
        blocker?.next_action ||
        "正式源 token 已配置后，还需要刷新并补齐当日 formal manifest。",
      href: "/settings#formal-data",
      action: "看状态",
    };
  }
  ...
  const affected = uniqueValues(
    sources
      .filter(
        (source) =>
          (source.manifest_path || source.has_manifest) &&
  ...
  return {
    ...
    title: affected.length
      ? `${affected.slice(0, 3).join("、")} 仍用快源/管线口径`
      : "正式数据口径尚未通过",
...
function accountGapSummary(account?: AccountReadinessState | null) {
  ...
  if (account.mode === "live_small" && account.ready_for_live_small) {
    return {
      ...
      detail: account.reconciliation?.age_label
        ? `最近对账 ${account.reconciliation.age_label}。`
        : "账户模式与对账均已通过。",
  ...
  if (account.mode === "live_small") {
    const reason =
      account.blockers?.[0]?.message ||
      "live_small 需要现金、对账和未完成动作全部通过。";
    return {
      tone: "risk",
      badge: "实盘未就绪",
...
function formalStatusReason(readiness?: ReadinessPayload | null) {
  ...
  const keys = [
    ...(blocker?.blocked_request_keys || []),
    ...(blocker?.missing_request_keys || []),
  ];
...
function visibleBlockingReasons(
  trust: TrustLevel,
  ...
  const filtered = source.filter(
    (reason) =>
      !reason.startsWith("正式口径未接入") &&
      !reason.includes("正式数据口径尚未通过"),
  );
...
export function TrustBanner({
  ...
  return (
    <section
      className={cn(
        "surface-card flex flex-col gap-3 p-4 lg:flex-row lg:items-start lg:justify-between",
        className,
      )}
      style={{
              ...
              {formalGap ? (
                <div className="rounded-md border border-[var(--border-subtle)] bg-[var(--bg-secondary)] px-3 py-2">
                  <div className="flex items-center gap-2">
                    <Database
                      size={13}
                      style={{ color: toneColor(formalGap.tone) }}
                    />
                    <span
                  ...
                  {"href" in formalGap && formalGap.href ? (
                    <Link
                      href={formalGap.href}
                      className="mt-2 inline-flex text-[11px] text-[var(--text-tertiary)] hover:text-[var(--text-primary)]"
                    >
                      {formalGap.action}
                    </Link>
                  ) : null}
                ...
                <div className="rounded-md border border-[var(--border-subtle)] bg-[var(--bg-secondary)] px-3 py-2">
                  <div className="flex items-center justify-between gap-2">
                    <div className="flex min-w-0 items-center gap-2">
                      <WalletCards
                        size={13}
                        style={{ color: toneColor(accountGap.tone) }}
                      />
                      <span
                    ...
                    {"href" in accountGap && accountGap.href ? (
                      <Link
                        href={accountGap.href}
                        className="shrink-0 text-[11px] text-[var(--text-tertiary)] hover:text-[var(--text-primary)]"
                      >
                        {accountGap.action}
                      </Link>
                    ) : null}
